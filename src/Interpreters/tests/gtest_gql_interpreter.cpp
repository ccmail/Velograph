...
class TestGraphStorage final : public IGraphStorage {
 public:
  explicit TestGraphStorage(const String &database_name) : IGraphStorage(StorageID{database_name, "_graph"}) {
 ...
 protected:
  const Block &getGraphHeader(GraphElementKind /*kind*/) const override { return vertex_header; }

  Pipe scanImpl(const SharedHeader & /*header*/, const IColumn::Filter & /*header_filter*/, GraphElementKind /*kind*/,
...
auto withRegisteredGraphStorage(const GraphStoragePtr &storage, Callable &&callable) {
  ...
  if (!database) throw Exception(ErrorCodes::LOGICAL_ERROR, "Current test database is not registered");
  if (database->isTableExist("_graph", context)) throw Exception(ErrorCodes::LOGICAL_ERROR, "Test graph storage is already registered");
...
QueryPlan buildPlanWithAnalyzer(const std::string &query, const GQLQueryOptions &options = {}) {
  ...
  return withRegisteredGraphStorage(storage, [&] {
...
QueryPlan makeSingleVertexMatchPlan(const GraphStoragePtr &storage) {
  QueryPlan plan;
  plan.addStep(std::make_unique<Graph::MatchStep>(makeSingleVertexMatchSpec("n"), storage, Names{"n"}, getInterpreterContext()));
  return plan;
}
...
QueryPlan buildPlanWithGQLPlanBuilder(const std::string &query) {
  ...
  if (!single_query) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "test query must parse to GQLSingleQuery");
...
const Graph::MatchStep *leafMatchStep(const QueryPlan &plan) {
  const auto *node = plan.getRootNode();
  while (node && node->children.size() == 1) node = node->children.front();
...
void collectMatchSteps(const QueryPlan::Node *node, std::vector<const Graph::MatchStep *> &steps) {
  ...
  if (const auto *match_step = dynamic_cast<const Graph::MatchStep *>(node->step.get())) steps.push_back(match_step);

  for (const auto &child : node->children) collectMatchSteps(child, steps);
}
...
TEST(GQLInterpreter, GQLPlanBuilderPlansSingleQueryWithoutInterpreter) {
  ...
  EXPECT_EQ(linearStepNames(plan), (std::vector<String>{"Limit", "Expression", "Filter", "GraphMatch"}));
}
...
TEST(GQLInterpreter, InlineCallImportsExpressionBindingFromOuterScope) {
  ...
  EXPECT_EQ(linearStepNames(plan), (std::vector<String>{"Expression", "Expression", "Expression", "ReadFromPreparedSource"}));
...
TEST(GQLInterpreter, SelectFromSubqueryReusesWhereProjectionAndPagePlanning) {
  ...
  EXPECT_EQ(linearStepNames(plan),
...
TEST(GQLInterpreter, NativeClickHouseAggregateFunctionUsesReusableAggregationDetection) {
  ASTs items;
  items.push_back(make_intrusive<GAST::GQLAliasedItem>(makeASTFunction("sum", make_intrusive<ASTIdentifier>("x")), "s"));
...
TEST(GQLInterpreter, NativeClickHouseNonCountAggregateRequiresArguments) {
  ...
  EXPECT_THROW(GQL::AggregationPlannerDetail::getAggregateFunctionInfo(*aggregate), DB::Exception);
}
...
TEST(GQLInterpreter, NativeClickHouseFunctionUsesReusableExpressionPlanner) {
  ...
  inputs.emplace_back("x", std::make_shared<DataTypeUInt64>());
  ActionsDAG dag(inputs);
  auto function = makeASTFunction("plus", make_intrusive<ASTIdentifier>("x"), make_intrusive<ASTLiteral>(Field(UInt64(1))));
...
TEST(GQLQueryTreeAnalyzer, GetSampleBlockReturnsHeaderWithoutExecuting) {
  auto &context = getMutableContext().context;
  auto storage = std::make_shared<TestGraphStorage>(context->getCurrentDatabase());
  const auto header = withRegisteredGraphStorage(storage, [&] {
...
TEST(GQLInterpreter, OptionalMatchThrowsNotImplemented) {
  expectExceptionCode([] { (void)buildPlanWithAnalyzer("OPTIONAL MATCH (n) RETURN n"); }, ErrorCodes::NOT_IMPLEMENTED);
}
...
TEST(GQLInterpreter, ExpandMatchStepsEdgePatternThrowsNotImplemented) {
  ...
  auto step = std::make_unique<Graph::MatchStep>(std::move(spec), storage, Names{"a", "e", "b"}, getInterpreterContext());

  QueryPlan::Nodes nodes;
  auto &plan_node = nodes.emplace_back();
  plan_node.step = std::move(step);

  expectExceptionCode([&] { QueryPlanOptimizations::expandMatchSteps(plan_node, nodes); }, ErrorCodes::NOT_IMPLEMENTED);
}
...
TEST(GQLInterpreter, MatchSingleVertexReturnsRealData) {
  ...
  auto ids = withRegisteredGraphStorage(graph_storage, [&] {
    ...
    InterpreterGQLQueryAnalyzer interpreter(parseGQL("MATCH (n) RETURN n"), mutable_context, GQLQueryOptions{});
    ...
    while (executor.pull(output)) {
      ...
      for (size_t i = 0; i < output.rows(); ++i) result.push_back(column.getElement(i));
    }
    return result;
  });
...
TEST(GQLQueryTreeAnalyzer, MatchPropertyColumnsBuildDeterministicHeader) {
  auto &context = getMutableContext().context;
  auto storage = std::make_shared<GraphStorageEngine>(context->getCurrentDatabase(), context);
  storage->registerVertexType("Person", {
                                            {"age", std::make_shared<DataTypeUInt64>()},
                                            {"name", std::make_shared<DataTypeString>()},
                                        });

  withRegisteredGraphStorage(storage, [&] {
    InterpreterGQLQueryAnalyzer interpreter(parseGQL("MATCH (a) WHERE a.age > 20 RETURN a.name"), context);
    auto plan = std::move(interpreter).extractQueryPlan();
    const auto *match_step = leafMatchStep(plan);
    ASSERT_NE(match_step, nullptr);

    const auto &header = *match_step->getOutputHeader();
    ASSERT_EQ(header.columns(), 3u);
    EXPECT_EQ(header.getByPosition(0).name, "a");
    EXPECT_EQ(header.getByPosition(1).name, "a.age");
    EXPECT_EQ(header.getByPosition(2).name, "a.name");

    expectExceptionCode(
        [&] {
          InterpreterGQLQueryAnalyzer unknown_property(parseGQL("MATCH (a) RETURN a.missing"), context);
          (void)std::move(unknown_property).extractQueryPlan();
        },
...
TEST(GQLInterpreter, MatchPropertyPredicatesNormalizeAndExecute) {
  ...
  withRegisteredGraphStorage(storage, [&] {
    ...
    labels_column->insert(Array{"Other"});
    ...
    auto name_column = name_type->createColumn();
    name_column->insert("Low");
    name_column->insert("Alice");
    name_column->insert("Bob");

    Block input;
    input.insert({std::move(id_column), std::make_shared<DataTypeUInt64>(), "__ID__"});
    input.insert({std::move(labels_column), labels_type, "labels"});
    input.insert({std::move(age_column), std::make_shared<DataTypeUInt64>(), "age"});
    input.insert({std::move(name_column), name_type, "name"});
    storage->writeVertex(input);

    auto execute_names = [&](const String &query) {
      InterpreterGQLQueryAnalyzer interpreter(parseGQL(query), context);
      auto block_io = interpreter.execute();
      PullingPipelineExecutor executor(block_io.pipeline);
      Block output;
      ...
      while (executor.pull(output)) {
        ...
        for (size_t i = 0; i < output.rows(); ++i) result.emplace_back(column.getDataAt(i));
      }
      std::sort(result.begin(), result.end());
      return result;
    };

    EXPECT_EQ(execute_names("MATCH (a) WHERE a.age > 20 RETURN a.name"), (std::vector<String>{"Alice", "Bob"}));

    const std::vector<String> expected{"Alice"};
    EXPECT_EQ(execute_names("MATCH (a) WHERE a.age = 30 RETURN a.name"), expected);
    EXPECT_EQ(execute_names("MATCH (a WHERE a.age = 30) RETURN a.name"), expected);
    EXPECT_EQ(execute_names("MATCH (a {age: 30}) RETURN a.name"), expected);
    EXPECT_EQ(execute_names("MATCH (a:Selected) RETURN a.name"), expected);
  });
}
