...
GraphStoragePtr resolveActiveGraphStorage(const Graph::MatchSpec & spec, ContextPtr context)
{
    ...
    if (spec.graph_reference)
    {
        ...
        throw Exception(
            ...
            "Graph reference resolution is not implemented in the analyzer planner yet");
    }
    else
    {
        /// M1 minimal resolution: the current database is the graph name.
        graph_name = context->getCurrentDatabase();
    }
    ...
    if (!storage)
        throw Exception(
            ...
            "No active graph for MATCH: graph '{}' has no registered graph storage",
            graph_name);
    ...
    if (!graph_storage)
        throw Exception(
            ErrorCodes::UNKNOWN_TABLE,
            "Table {}._graph is not a graph storage",
            graph_name);
