...
Names getHeaderColumnNames(const Block& header) {
  ...
  for (size_t i = 0; i < header.columns(); ++i) names.push_back(header.getByPosition(i).name);

  return names;
}

CombinedPlanMode getCombinedPlanMode(const GAST::GQLCombinedQuery& query) {
  if (query.queries.size() < 2) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL combined query must contain at least two subqueries");
  ...
  for (const auto operation : query.operators) {
    ...
    if (mode && *mode != current_mode) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Mixing GQL combined query operators is not supported");

    mode = current_mode;
  }

  if (!mode) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL combined query has no operator");
...
bool needsDistinctStep(CombinedPlanMode mode) {
  return mode == CombinedPlanMode::UnionDistinct || mode == CombinedPlanMode::ExceptDistinct || mode == CombinedPlanMode::IntersectDistinct;
}

void buildGQLQueryPlanImpl(QueryPlan& query_plan, const IAST& query, ContextPtr context, const PlanScope* initial_scope,
                           PlanScope* output_scope);

QueryPlanPtr buildChildPlan(const ASTPtr& query, ContextPtr context, const PlanScope* initial_scope) {
  if (!query) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL combined child query is null");
...
void addConversionIfNeeded(QueryPlan& plan, const SharedHeader& result_header, ContextPtr context) {
  if (blocksHaveEqualStructure(*plan.getCurrentHeader(), *result_header)) return;

  auto actions =
      ActionsDAG::makeConvertingActions(plan.getCurrentHeader()->getColumnsWithTypeAndName(), result_header->getColumnsWithTypeAndName(),
...
void buildSingleQueryPlan(QueryPlan& query_plan, const GAST::GQLSingleQuery& query, ContextPtr context, const PlanScope* initial_scope,
                          PlanScope* output_scope) {
  if (initial_scope) {
    GQLPlanBuilder builder(context, *initial_scope);
    builder.buildSingleQuery(query_plan, query);
    if (output_scope) *output_scope = builder.getScope();
    return;
  }
...
void buildCombinedQueryPlan(QueryPlan& query_plan, const GAST::GQLCombinedQuery& query, ContextPtr context, const PlanScope* initial_scope,
                            PlanScope* output_scope) {
  ...
  for (const auto& child : query.queries) plans.push_back(buildChildPlan(child, context, initial_scope));
  ...
  if (needsDistinctStep(mode)) addDistinctStep(query_plan, result_columns, context);

  if (output_scope) output_scope->replaceWithHeader(*query_plan.getCurrentHeader(), BindingKind::Projection);
}
...
Graph::MatchPathSpec buildPathSpecFromTree(const GQLPathPatternNode& path) {
  ...
  if (!term) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL analyzer planner only supports classic path terms");
  ...
  for (size_t i = 0; i < elements.size(); ++i) {
    if (i % 2 == 0) {
      ...
      if (!node) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL path factor must be GQLNodePatternNode");
      if (node->getLabelExpression() || node->getPropertyMap() || node->getWhere())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL node predicates must be removed by GQLPredicateNormalizationPass before planning");
      ...
      if (!edge) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL path factor must be GQLEdgePatternNode");
      if (edge->getLabelExpression() || edge->getPropertyMap() || edge->getWhere())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL edge predicates must be removed by GQLPredicateNormalizationPass before planning");
...
Graph::MatchSpec buildMatchSpecFromTree(const GQLMatchNode& match_node) {
  if (match_node.isOptional()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "OPTIONAL MATCH is not supported in GQL analyzer planner");
  ...
  if (patterns.empty()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL MATCH must contain at least one path pattern");

  Graph::MatchClauseSpec clause;
  clause.paths.reserve(patterns.size());
  for (const auto& pattern : patterns) {
    const auto* path = pattern->as<GQLPathPatternNode>();
    if (!path) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL MATCH path pattern must be GQLPathPatternNode");
...
void collectColumnReferences(const QueryTreeNodePtr& node, const IQueryTreeNode* source, std::unordered_set<String>& referenced) {
  if (!node) return;

  if (const auto* column = node->as<ColumnNode>()) {
    const auto column_source = column->getColumnSourceOrNull();
    if (column_source && column_source.get() == source) referenced.insert(column->getColumnName());
  }

  for (const auto& child : node->getChildren()) collectColumnReferences(child, source, referenced);
}
...
Names collectReferencedColumns(const GQLLinearQueryNode& linear, const GQLMatchNode& match) {
  ...
  for (const auto& step : linear.getSteps().getNodes()) collectColumnReferences(step, &match, referenced);
  ...
  for (const auto& binding : collectMatchBindings(match)) {
    ...
    for (const auto& name : referenced) {
      if (name.starts_with(prefix)) properties.push_back(name);
    }
...
void planMatchFromTree(QueryPlan& plan, const GQLMatchNode& match_node, Names referenced_columns, ContextPtr context, PlanScope& scope) {
  ...
  plan.addStep(std::make_unique<Graph::MatchStep>(std::move(match_spec), std::move(graph_storage), std::move(referenced_columns), context));
  scope.replaceWithHeader(*plan.getCurrentHeader(), BindingKind::Source);
  ...
  if (const auto& where = match_node.getWhere()) planFilter(plan, where);
}
...
const ActionsDAG::Node& buildActionsNode(const QueryTreeNodePtr& expression, ActionsDAG& dag) {
  if (const auto* column = expression->as<ColumnNode>()) {
    ...
    if (!node)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL expression references binding '{}' not present in the current plan scope", name);
    return *node;
  }
  ...
  if (const auto* function = expression->as<FunctionNode>()) {
    ...
    for (const auto& argument : function_arguments) arguments.push_back(&buildActionsNode(argument, dag));
  ...
  throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL analyzer planner does not yet lower this expression node into actions");
}
...
void planReturnFromTree(QueryPlan& plan, const GQLReturnNode& ret, ContextPtr context, PlanScope& scope) {
  ...
  if (items.empty()) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL RETURN must project at least one item");
...
void planLinearQueryFromTree(QueryPlan& plan, const GQLLinearQueryNode& linear, ContextPtr context, PlanScope& scope) {
  ...
  for (const auto& step : steps) {
    if (!step) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL linear query step is null");

    if (const auto* match = step->as<GQLMatchNode>()) {
      if (source_planned) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL analyzer planner does not yet support multiple source clauses");
    ...
    if (const auto* filter = step->as<GQLFilterNode>()) {
      if (!source_planned)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL analyzer planner does not support FILTER before a source clause");
  ...
  if (!source_planned) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL analyzer planner requires a source clause");
}
...
CombinedPlanMode getCombinedPlanModeFromTree(const GQLCombinedQueryNode& combined) {
  ...
  if (queries.size() < 2) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL combined query must contain at least two subqueries");
...
void planCombinedQueryFromTree(QueryPlan& plan, const GQLCombinedQueryNode& combined, ContextPtr context) {
  ...
  for (const auto& child : queries) {
    if (!child) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL combined query child is null");
  ...
  if (needsDistinctStep(mode)) addDistinctStep(plan, result_columns, context);
}
...
Planner::Planner(const QueryTreeNodePtr& query_tree_, const ContextPtr& context_, const GQLQueryOptions& options_)
    : query_tree(query_tree_), context(context_), options(options_) {}

Planner::Planner(const QueryTreeNodePtr& query_tree_, const ContextPtr& context_, const GQLQueryOptions& options_,
                 PlanScope& initial_scope_)
    : query_tree(query_tree_), context(context_), options(options_), plan_scope(initial_scope_) {}
...
void Planner::buildPlanForLinearQueryNode() {
  ...
  if (!linear) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL query tree is not a linear query");
...
void Planner::buildPlanForCombinedQueryNode() {
  ...
  if (!combined) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL query tree is not a combined query");

  planCombinedQueryFromTree(query_plan, *combined, context);
}

void Planner::buildQueryPlanIfNeeded() {
  if (query_plan_built) return;
  ...
  switch (query_tree->getNodeType()) {
    ...
    case QueryTreeNodeType::GQL_COMBINED_QUERY:
      ...
      break;
    default:
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL planner does not yet support query tree node type: {}", query_tree->getNodeType());
  }
