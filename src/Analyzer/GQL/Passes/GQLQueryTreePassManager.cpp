  ...
  void visitImpl(const QueryTreeNodePtr& node) {
    if (const auto* column = node->as<ColumnNode>()) {
      if (!column->getColumnSourceOrNull())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL ColumnNode '{}' has no source after pass '{}'", column->getColumnName(), pass_name);
    } else if (const auto* function = node->as<FunctionNode>()) {
      if (!function->isResolved())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL FunctionNode '{}' is not resolved after pass '{}'", function->getFunctionName(),
                        pass_name);
    }
  }
...
void GQLQueryTreePassManager::addPass(QueryTreePassPtr pass) { passes.push_back(std::move(pass)); }

void GQLQueryTreePassManager::run(QueryTreeNodePtr& query_tree_node) { run(query_tree_node, passes.size()); }

void GQLQueryTreePassManager::run(QueryTreeNodePtr& query_tree_node, size_t up_to_pass_index) {
  if (up_to_pass_index > passes.size())
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "GQL pass index {} exceeds passes count {}", up_to_pass_index, passes.size());
...
void GQLQueryTreePassManager::runOnlyResolve(QueryTreeNodePtr& query_tree_node) { run(query_tree_node, resolution_passes_count); }
