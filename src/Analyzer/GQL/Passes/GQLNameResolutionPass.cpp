...
void resolveExpression(QueryTreeNodePtr &node, const GQLMatchBindings &bindings, const QueryTreeNodePtr &source,
  ...
  if (const auto *property = node->as<GQLPropertyAccessNode>()) {
    ...
    if (!base) throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "GQL property access base must be a visible MATCH variable");

    const auto variable_name = base->getIdentifier().getFullName();
    const auto *binding = findMatchBinding(bindings, variable_name);
    if (!binding) throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Unknown GQL MATCH variable '{}' in property access", variable_name);

    auto column = makePropertyColumn(*binding, property->getPropertyName(), source, context);
    if (property->hasAlias()) column->setAlias(property->getAlias());
  ...
  if (const auto *identifier = node->as<IdentifierNode>()) {
    ...
    auto column = std::make_shared<ColumnNode>(NameAndTypePair{name, std::make_shared<DataTypeUInt64>()}, source);
    if (identifier->hasAlias()) column->setAlias(identifier->getAlias());
  ...
  if (auto *function = node->as<FunctionNode>()) {
    ...
    for (auto &argument : function->getArguments().getNodes()) resolveExpression(argument, bindings, source, context);

    if (!function->isResolved()) function->resolveAsFunction(FunctionFactory::instance().get(function->getFunctionName(), context));
  }
}

void resolveReturnItems(GQLReturnNode &ret, const GQLMatchBindings &bindings, const QueryTreeNodePtr &source, const ContextPtr &context) {
  for (auto &item : ret.getItems().getNodes()) resolveExpression(item, bindings, source, context);
}

void resolvePatternExpressions(GQLMatchNode &match, const GQLMatchBindings &bindings, const QueryTreeNodePtr &source,
                               const ContextPtr &context) {
  for (auto &pattern : match.getPathPatterns().getNodes()) {
    ...
    for (auto &element : term->getElements().getNodes()) {
      ...
      if (*property_map) {
        ...
        for (auto &item : items) resolveExpression(item->as<GQLPropertyItemNode &>().getValue(), bindings, source, context);
      }
...
void resolveLinearQuery(GQLLinearQueryNode &linear, const ContextPtr &context) {
  ...
  for (auto &step : linear.getSteps().getNodes()) {
    if (auto *match = step->as<GQLMatchNode>()) {
      ...
      if (match->getWhere()) resolveExpression(match->getWhere(), bindings, source, context);
...
void resolveCombinedQuery(GQLCombinedQueryNode &combined, const ContextPtr &context) {
  for (auto &query : combined.getQueries().getNodes()) resolveQuery(query, context);
}
...
void GQLNameResolutionPass::run(QueryTreeNodePtr &query_tree_node, ContextPtr context) { resolveQuery(query_tree_node, context); }
