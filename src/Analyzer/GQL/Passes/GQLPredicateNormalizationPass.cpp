...
QueryTreeNodePtr combineConjuncts(QueryTreeNodes conjuncts, const ContextPtr &context)
{
    ...
    for (size_t i = 1; i < conjuncts.size(); ++i)
        result = makeFunction("and", {std::move(result), std::move(conjuncts[i])}, context);
    return result;
}
                                          ...
                                          const QueryTreeNodePtr &source, const ContextPtr &context)
{
    ...
    {
        if (label.getLabelName().empty())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL label predicate has an empty label name");
        ...
        return makeFunction(binding.kind == GraphElementKind::Vertex ? "has" : "equals", {std::move(property), std::move(label_name)},
                            context);
    }
    ...
    if (label.getOperator() == Operator::Not)
    {
        if (operands.size() != 1 || !operands.front())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL label negation must have one operand");
        return makeFunction(
            "not", {normalizeLabelExpression(operands.front()->as<const GQLLabelExpressionNode &>(), binding, source, context)}, context);
    }
                             ...
                             const ContextPtr &context, QueryTreeNodes &conjuncts)
{
    ...
    if (inline_where)
        conjuncts.push_back(std::move(inline_where));
    ...
    if (property_map)
    {
        ...
        for (const auto &item_node : items)
        {
            ...
            if (!item.getValue())
                throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL property map item has no value");
...
void normalizeMatch(GQLMatchNode &match, const QueryTreeNodePtr &source, const ContextPtr &context)
{
    ...
    if (match.getWhere())
        conjuncts.push_back(std::move(match.getWhere()));
    ...
    for (auto &pattern : match.getPathPatterns().getNodes())
    {
        ...
        for (auto &element : term->getElements().getNodes())
        {
            ...
            {
                appendElementPredicates(node->getElementVariable(), node->getLabelExpression(), node->getPropertyMap(), node->getWhere(),
                                        bindings, source, context, conjuncts);
            }
            else if (auto *edge = element->as<GQLEdgePatternNode>())
            {
                appendElementPredicates(edge->getElementVariable(), edge->getLabelExpression(), edge->getPropertyMap(), edge->getWhere(),
                                        bindings, source, context, conjuncts);
            }
        }
    }
...
void normalizeQuery(QueryTreeNodePtr &query, const ContextPtr &context)
{
    if (auto *linear = query ? query->as<GQLLinearQueryNode>() : nullptr)
    {
        for (auto &step : linear->getSteps().getNodes())
        {
            if (auto *match = step->as<GQLMatchNode>())
                normalizeMatch(*match, step, context);
        }
        return;
    }
    ...
    {
        for (auto &child : combined->getQueries().getNodes())
            normalizeQuery(child, context);
    }
}

}  // namespace

void GQLPredicateNormalizationPass::run(QueryTreeNodePtr &query_tree_node, ContextPtr context)
{
    normalizeQuery(query_tree_node, context);
}
