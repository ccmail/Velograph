...
std::pair<Field, DataTypePtr> parseGQLLiteral(const String& raw) {
  ...
  if (upper == "TRUE") return {Field(UInt64(1)), std::make_shared<DataTypeUInt8>()};
  if (upper == "FALSE") return {Field(UInt64(0)), std::make_shared<DataTypeUInt8>()};
  if (upper == "NULL") return {Field(), std::make_shared<DataTypeNullable>(std::make_shared<DataTypeNothing>())};
  ...
  try {
    if (text.find_first_of(".eE") != String::npos) return {Field(parseFromString<Float64>(text)), std::make_shared<DataTypeFloat64>()};
    if (text.starts_with('-')) return {Field(parseFromString<Int64>(text)), std::make_shared<DataTypeInt64>()};
...
const char* binaryOperatorToFunctionName(const String& op) {
  if (op == "=") return "equals";
  if (op == "<>" || op == "!=") return "notEquals";
  if (op == ">") return "greater";
  if (op == ">=") return "greaterOrEquals";
  if (op == "<") return "less";
  if (op == "<=") return "lessOrEquals";
  if (op == "+") return "plus";
  if (op == "-") return "minus";
...
class GQLQueryTreeBuilderImpl {
 public:
  ...
  QueryTreeNodePtr build(const IAST& query) {
    if (const auto* single = query.as<GAST::GQLSingleQuery>()) return buildLinearQuery(*single);

    if (const auto* combined = query.as<GAST::GQLCombinedQuery>()) return buildCombinedQuery(*combined);
 ...
 private:
  QueryTreeNodePtr buildLinearQuery(const GAST::GQLSingleQuery& query) {
    ...
    for (const auto& clause : query.clauses) {
      if (auto step = buildClause(clause)) steps_list->getNodes().push_back(step);
    }
  ...
  QueryTreeNodePtr buildCombinedQuery(const GAST::GQLCombinedQuery& query) {
    ...
    for (const auto& subquery : query.queries) {
      if (!subquery) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL combined query has null subquery");
    ...
    for (auto op : query.operators) operators.push_back(convertOperator(op));
  ...
  QueryTreeNodePtr buildClause(const ASTPtr& clause) {
    ...
    if (const auto* match = clause->as<GAST::GQLMatchClause>()) return buildMatchClause(*match);

    if (const auto* where = clause->as<GAST::GQLWhereClause>()) return buildFilterClause(*where);

    if (const auto* ret = clause->as<GAST::GQLReturnClause>()) return buildReturnClause(*ret);

    if (const auto* order_by = clause->as<GAST::GQLOrderByClause>()) return buildOrderByClause(*order_by);

    if (const auto* page = clause->as<GAST::GQLPageClause>()) return buildPageClause(*page);
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL clause not yet supported in QueryTree builder: {}", clause->getID(' '));
  }
  ...
  QueryTreeNodePtr buildMatchClause(const GAST::GQLMatchClause& match) {
    ...
    for (const auto& pattern : match.path_patterns) {
      if (!pattern) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL MATCH path pattern is null");

      const auto* path = pattern->as<GAST::GQLPathPattern>();
      if (!path) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL MATCH path pattern must be GQLPathPattern, got {}", pattern->getID(' '));
    ...
    if (match.keep_clause) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL MATCH ... KEEP is not yet supported in QueryTree builder");
    ...
    if (match.optional_operand_block)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL OPTIONAL MATCH operand block is not yet supported in QueryTree builder");
  ...
  static String identifierVariable(const GAST::Ptr& variable) {
    ...
    if (!expr || expr->kind != GAST::GQLExpr::Kind::Identifier) return {};
  ...
  QueryTreeNodePtr buildLabelExpression(const GAST::GQLLabelExpression& label) {
    ...
    for (const auto& child : label.children) {
      ...
      if (!child_label) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL label expression has an invalid operand");
  ...
  QueryTreeNodePtr buildPropertyMap(const GAST::GQLPropertyMap& map) {
    ...
    for (const auto& item_ast : map.items) {
      ...
      if (!item || !item->value) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL property map has an invalid item");
  ...
  QueryTreeNodePtr buildElementWhere(const GAST::Ptr& where_ast) {
    ...
    if (!where || !where->expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL element WHERE must contain a predicate");
  ...
  QueryTreeNodePtr buildPatternLabel(const GAST::Ptr& label_ast) {
    ...
    if (!label) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL pattern label has an invalid AST shape");
  ...
  QueryTreeNodePtr buildPatternProperties(const GAST::Ptr& properties_ast) {
    ...
    if (!properties) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL pattern property map has an invalid AST shape");
  ...
  QueryTreeNodePtr buildNodePattern(const GAST::GQLNodePattern& node) {
    ...
    if (node.label_expression) node_pattern->getLabelExpression() = buildPatternLabel(node.label_expression);
    if (node.properties) node_pattern->getPropertyMap() = buildPatternProperties(node.properties);
    if (node.where) node_pattern->getWhere() = buildElementWhere(node.where);
  ...
  QueryTreeNodePtr buildEdgePattern(const GAST::GQLEdgePattern& edge) {
    ...
    if (edge.label_expression) edge_pattern->getLabelExpression() = buildPatternLabel(edge.label_expression);
    if (edge.properties) edge_pattern->getPropertyMap() = buildPatternProperties(edge.properties);
    if (edge.where) edge_pattern->getWhere() = buildElementWhere(edge.where);
    if (edge.quantifier) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL edge quantifier is not yet supported in QueryTree builder");
  ...
  QueryTreeNodePtr buildPathTerm(const GAST::GQLPathTerm& term) {
    if (term.factors.empty() || term.factors.size() % 2 == 0)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL path term must contain an odd number of alternating node and edge factors");
    ...
    for (size_t i = 0; i < term.factors.size(); ++i) {
      ...
      if (!factor) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL path factor is null");

      if (i % 2 == 0) {
        const auto* node = factor->as<GAST::GQLNodePattern>();
        if (!node) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL path factor must be GQLNodePattern, got {}", factor->getID(' '));
        elements->getNodes().push_back(buildNodePattern(*node));
      } else {
        const auto* edge = factor->as<GAST::GQLEdgePattern>();
        if (!edge) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL path factor must be GQLEdgePattern, got {}", factor->getID(' '));
  ...
  QueryTreeNodePtr buildPathExpression(const GAST::Ptr& expression) {
    if (!expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL path pattern has no expression");

    if (const auto* term = expression->as<GAST::GQLPathTerm>()) return buildPathTerm(*term);

    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL path expression {} is not yet supported in QueryTree builder",
                    expression->getID(' '));
  }
  ...
  QueryTreeNodePtr buildPathPattern(const GAST::GQLPathPattern& path) {
    ...
    if (path.prefix) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL path prefix is not yet supported in QueryTree builder");
  ...
  QueryTreeNodePtr buildExpression(const IAST& expr) {
    if (const auto* gql_expr = expr.as<GAST::GQLExpr>()) {
      if (gql_expr->kind == GAST::GQLExpr::Kind::Identifier) return std::make_shared<IdentifierNode>(Identifier(gql_expr->text));
      ...
      if (gql_expr->kind == GAST::GQLExpr::Kind::BinaryOp) return buildBinaryOp(*gql_expr);
    ...
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL expression {} is not yet supported in QueryTree builder", expr.getID(' '));
  }
  ...
  QueryTreeNodePtr buildBinaryOp(const GAST::GQLExpr& expr) {
    ...
    if (!function_name)
      throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL binary operator '{}' is not yet supported in QueryTree builder", expr.text);
  ...
  QueryTreeNodePtr buildFilterClause(const GAST::GQLWhereClause& where) {
    if (!where.expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL FILTER/WHERE clause must have a predicate");
  ...
  QueryTreeNodePtr buildReturnClause(const GAST::GQLReturnClause& ret) {
    if (ret.return_all) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL RETURN * is not yet supported in QueryTree builder");
    if (ret.group_by) throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL RETURN ... GROUP BY is not yet supported in QueryTree builder");
    ...
    for (const auto& item_ast : ret.items) {
      if (!item_ast) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL RETURN item is null");

      const auto* item = item_ast->as<GAST::GQLAliasedItem>();
      if (!item) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL RETURN item must be GQLAliasedItem, got {}", item_ast->getID(' '));
      if (!item->expression) throw Exception(ErrorCodes::LOGICAL_ERROR, "GQL RETURN item has no expression");

      auto expr_node = buildExpression(*item->expression);
      if (!item->alias.empty()) expr_node->setAlias(item->alias);
  ...
  QueryTreeNodePtr buildOrderByClause(const GAST::GQLOrderByClause& order_by) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL ORDER BY is not yet supported in QueryTree builder: {}", order_by.getID(' '));
  }

  QueryTreeNodePtr buildPageClause(const GAST::GQLPageClause& page) {
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "GQL OFFSET/LIMIT is not yet supported in QueryTree builder: {}", page.getID(' '));
  }
