...
{
    if (vertex_types.contains(name))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Graph vertex type '{}' is already registered", name);

    vertex_types.emplace(name, GraphVertexType{name, columns});
}

void GraphSchemaRegistry::registerEdgeType(
    const String & name, const NamesAndTypes & columns, const String & source_vertex_type, const String & target_vertex_type)
{
    if (edge_types.contains(name))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Graph edge type '{}' is already registered", name);
...
NamesAndTypes GraphSchemaRegistry::getVerticesTableColumns() const
{
    ...
    for (const auto & [_, vertex_type] : vertex_types)
        for (const auto & col : vertex_type.columns)
...
NamesAndTypes GraphSchemaRegistry::getEdgesTableColumns() const
{
    ...
    for (const auto & [_, edge_type] : edge_types)
        for (const auto & col : edge_type.columns)
            columns.push_back(col);
