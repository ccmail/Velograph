...
GraphStorageEngine::GraphStorageEngine(String graph_name_, ContextMutablePtr context_)
    : graph_name(std::move(graph_name_)), context(std::move(context_)) {}
...
void GraphStorageEngine::registerEdgeType(const String& type_name, const NamesAndTypes& columns, const String& source_vertex_type,
...
String GraphStorageEngine::verticesTableName() const { return graph_name + ".vertices"; }

String GraphStorageEngine::edgesForwardTableName() const { return graph_name + ".edges_forward"; }

String GraphStorageEngine::edgesReverseTableName() const { return graph_name + ".edges_reverse"; }

String GraphStorageEngine::vertexDegreesTableName() const { return graph_name + ".vertex_degrees"; }

// --- Internal table creation ---

void GraphStorageEngine::createTable(const String& table_name, const NamesAndTypes& columns, const String& order_by, const String& engine,
                                     bool is_materialized_view, const String& mv_source_table, const String& mv_select) {
  ...
  if (!order_by.empty()) {
    ...
    for (const auto& part : parts) {
      String trimmed = part;
      trimmed.trim();
      if (!trimmed.empty()) order_by_ast->children.push_back(std::make_shared<ASTIdentifier>(trimmed));
    }
...
void GraphStorageEngine::createInternalTables() {
  ...
  createTable(vertexDegreesTableName(), degree_columns, "__SRC__,type", "SummingMergeTree",
...
void GraphStorageEngine::dropInternalTables() {
  ...
  auto drop_by_name = [this](const String& unqualified_name) {
    InterpreterDropQuery::executeDropQuery(ASTDropQuery::Kind::Drop, context, context, StorageID{graph_name, unqualified_name},
                                           /*sync=*/false);
  };
...
StoragePtr GraphStorageEngine::getInternalStorage(const String& table_name) const {
  ...
  if (!storage) throw Exception(ErrorCodes::UNKNOWN_TABLE, "Internal graph table '{}' is not found", table_name);
  return storage;
}
...
Pipe GraphStorageEngine::readForwardEdges(UInt64 src, const String& edge_type, ContextPtr read_context) const {
  ...
  /// Get all columns
  Names column_names;
  for (const auto& col : storage->getInMemoryMetadataPtr()->getColumns().getAll()) column_names.push_back(col.name);

  QueryPlan plan;
  storage->read(plan, column_names, snapshot, query_info, read_context, QueryProcessingStage::Complete,
                8192,  /// max_block_size
                1);    /// num_streams

  return *plan.buildQueryPipeline(QueryPlanOptimizationSettings(read_context), BuildQueryPipelineSettings(read_context));
}
...
Block GraphStorageEngine::readVertex(UInt64 id, ContextPtr read_context) const {
  ...
  auto pipeline = plan.buildQueryPipeline(QueryPlanOptimizationSettings(read_context), BuildQueryPipelineSettings(read_context));

  auto executor = std::make_unique<PullingPipelineExecutor>(*pipeline);
  Block result;
  Block block;
  while (executor->pull(block)) {
    if (!result) result = block.cloneEmpty();
  }
  return result;
}
