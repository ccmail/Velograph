 ...
 *
 * Reads are designed for graph traversal:
 *   readForwardEdges(src, type)  — one-hop out-edges
 *   readReverseEdges(dst, type)  — one-hop in-edges
 *   readVertex(id)               — single vertex lookup
 *   getDegree(src, type)         — out-degree for query planning
 */
class GraphStorageEngine {
 public:
  ...
  /// The database must already exist in DatabaseCatalog.
