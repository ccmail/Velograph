...
SharedHeader MatchVertexStep::makeHeader(const GraphStoragePtr &storage, const String &variable, const Names &referenced_columns) {
  Block header;
  ...
  for (const auto &column_name : referenced_columns) {
    ...
    if (!column_name.starts_with(prefix)) continue;

    const auto property_name = column_name.substr(prefix.size());
    const auto &property = storage_header.getByName(property_name);
    header.insert({property.type, column_name});
  }

  return std::make_shared<const Block>(std::move(header));
}

MatchVertexStep::MatchVertexStep(GraphStoragePtr storage_, String variable_, Names referenced_columns_, ContextPtr context_)
...
void MatchVertexStep::initializePipeline(QueryPipelineBuilder &pipeline, const BuildQueryPipelineSettings &) {
  ...
  for (const auto &column_name : referenced_columns) {
    if (column_name == variable)
      ...
      storage_projection.insert(column_name.substr(prefix.size()));
  }

  auto pipe = storage->scan(storage_projection, GraphElementKind::Vertex, DEFAULT_BLOCK_SIZE,
                            /*num_streams=*/1);
  ...
  for (const auto &output_column : *getOutputHeader()) {
    const auto storage_name = output_column.name == variable ? String(GRAPH_COL_ID) : output_column.name.substr(prefix.size());
    const auto *input = dag.tryFindInOutputs(storage_name);
    if (!input) throw Exception(ErrorCodes::LOGICAL_ERROR, "Graph vertex scan did not return projected column '{}'", storage_name);
  ...
  pipe.addSimpleTransform([&](const SharedHeader &header) { return std::make_shared<ExpressionTransform>(header, expression); });
...
void MatchVertexStep::describeActions(JSONBuilder::JSONMap &map) const { map.add("variable", variable); }
